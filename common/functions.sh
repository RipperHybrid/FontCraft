...
install_font() {
    ...
    ui_print "   >[Created destination directory for $font_name]< "
    sleep 0.5

    if [ "$font_name" = "Emoji" ]; then
        ui_print "   >[Renaming and installing emoji font...]< "
        cp "$font_path" "$MODPATH/system/fonts/NotoColorEmoji.ttf" || {
        ...
        for dest_file in SourceSansPro-Regular.ttf RobotoStatic-Regular.ttf Roboto-Regular.ttf DroidSansMono.ttf; do
            cp "$tmp_dir/selected_font.ttf" "$MODPATH/system/fonts/$dest_file" || {
                ui_print "   >[Error: Failed to copy $dest_file.]< "
                exit 1
            }
        done

        ui_print "   >[$font_name font successfully installed.]< "
    else
...
download_ef() {
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        ui_print "   >[No downloader found (curl/wget).]< "
        return 1
    fi

    if [ -s "$output_path" ]; then
        ui_print "   >[Successfully downloaded: $name]< "
        return 0
    else
...
download_json() {
    ...
    if [ -s "$JSON_PATH" ]; then
        ui_print "   >[✅ JSON downloaded successfully.]< "
    else
        ui_print "-    >[❌ Failed to download JSON.]< "
